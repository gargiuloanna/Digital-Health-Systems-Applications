    ...
    private void getPatientStudies() {
			...
			protected Task<List<Resource>> createTask() throws FhirClientConnectionException {
				return new Task<List<Resource>>() {
					...
					protected List<Resource> call() throws Exception {
						String id_patient = searchField.getText();
						return ServerInteraction.getPatientServiceRequests(id_patient);
					};
				};
			}
		};
		...
		getResource.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
			...
			public void handle(WorkerStateEvent event) {
				...
				if (r.size() == 0) {
					Alert alert = new Alert(AlertType.INFORMATION, "No requests found for this patient", ButtonType.OK);
					alert.showAndWait();
				} else
					for (Resource p : r)
						if (r != null)
